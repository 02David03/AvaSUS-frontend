export const Stars = ({rating, className='gap-4', ratingsNumber=''}) => {
  ...
  const StarPath = ({index}) => {
    if( rating/index >= 1 || index - rating <= 0.15 ) {
      ...
      { Array(5).fill('').map(( _, index) => {
        return(
          <svg key={index} xmlns="http://www.w3.org/2000/svg" fill='red' className="w-5 h-auto fill-current"
            viewBox="0 0 16 16">
              <StarPath index={index + 1} />
            </svg>
        );
      })}

      <p className={ (ratingsNumber !== '' && 'font-bold text-lg') + ' mt-1'}> 
        {rating.replace('.', ',')} {ratingsNumber !=='' && `(${ratingsNumber.toLocaleString()} avaliações)`} 
      </p>
